...
def to_run_url(path: str) -> str:
    rel = os.path.relpath(path, os.path.join(os.getcwd(), "runs")).replace("\\", "/")
    return f"/runs/{rel}"
...
class ProcessingRun(Base):
    ...
    paper_data = Column(JSONB, nullable=False)
...
async def process_endpoint(
    ...
    if SessionLocal:
        with SessionLocal() as db:
            rec = ProcessingRun(paper_data=paper_data)
            db.add(rec)
            db.commit()
            db.refresh(rec)
            db_id = rec.id

    imgs = paper_data.get("images", [])
    for img in imgs:
        if "path" in img:
            img["url"] = to_run_url(img["path"])
    paper_data["images"] = imgs

    run_dir = os.path.dirname(result.markdown_file)
    pdf_url = to_run_url(result.pdf_path)
    md_url = to_run_url(result.markdown_file)
    json_url = to_run_url(result.json_file)
...
def save_json(payload: PaperIn):
    if not SessionLocal:
        raise HTTPException(500, "DB not configured")
    with SessionLocal() as db:
        rec = ProcessingRun(paper_data=payload.data)
        db.add(rec)
        db.commit()
        db.refresh(rec)
        return {"id": rec.id}
...
def get_run(id: int):
    if not SessionLocal:
        raise HTTPException(500, "DB not configured")
    with SessionLocal() as db:
        rec = db.get(ProcessingRun, id)
        if not rec:
            raise HTTPException(404, "Not found")
        return {"id": rec.id, "paper_data": rec.paper_data}
